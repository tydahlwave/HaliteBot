...
public class MyBot {
    ...
    public static void main(String[] args) throws java.io.IOException {
        ...
        Networking.sendInit("tydahlwave");
    ...
    private static Move getMoveForLocation(final GameMap map, final Location loc) {
        ...
        // if (isInCombat) {
        //     Location nearestCombatPos = combatPositions.get(0);
        //     double minDist = map.width * map.height;
        //     for (Location combatPos : combatPositions) {
        //         double dist = map.getDistance(loc, combatPos);
        //         if (dist < minDist) {
        //             minDist = dist;
        //             nearestCombatPos = combatPos;
        //         }
        //     }
        //     chosenTarget = nearestCombatPos;
        // } else {
        ...
        // Only calculate A* if target is closer than 15 due to time efficiency problems
        if (map.getDistance(loc, chosenTarget) < 15) {
            direction = astar(map, loc, chosenTarget);
        } else {
            direction = getDirectionForTarget(map, loc, chosenTarget);
        }
    ...
    private static Direction checkValidMove(final GameMap map, final Location loc, final Direction dir) {
        ...
        // }
        // If cannot defeat neutral piece by moving in this direction, stay still
        if (targetSite.owner == neutralID && targetSite.strength >= mySite.strength) {
            return Direction.STILL;
        // } else if (targetSite.owner == myID && targetSite.strength >= mySite.strength) {
    ...
    private static Direction astar(final GameMap map, final Location begin, final Location end) {
        LinkedList<AStarNode> open = new LinkedList<>();
        LinkedList<AStarNode> closed = new LinkedList<>();

        AStarNode start = new AStarNode(begin, null);
        // Create node to compare completion state against
        AStarNode finish = new AStarNode(end, null);

        // Add initial nodes to the open list
        open.addAll(neighbors_astar(map, start));
        // Add start node to closed list
        closed.add(start);

        while (!open.isEmpty()) {
            // Pull the best node from the open list
            AStarNode bestNode = open.parallelStream()
                                     .sorted((n1, n2) -> Double.compare(heuristic_astar(map, end, n1), heuristic_astar(map, end, n2)))
                                     .findFirst()
                                     .get();
            open.remove(bestNode);

            // Check if at finish, and return direction to move
            if (bestNode.equals(finish)) {
                while (bestNode.parent.parent != null) {
                    bestNode = bestNode.parent;
                }
                // return bestNode;
                return getDirectionForTarget(map, bestNode.parent.loc, bestNode.loc);
            }

            // Iterate through neighbors of best node, adding to open list if new or better node
            for (AStarNode neighbor : neighbors_astar(map, bestNode)) {
                if (closed.contains(neighbor)) continue;
                // Get index of neighbor on open list if it exists
                int existingIndex = open.indexOf(neighbor);
                if (existingIndex >= 0) {
                    // If heuristic value of neighbor is better than existing node, replace it
                    AStarNode existingNode = open.get(existingIndex);
                    if (heuristic_astar(map, end, neighbor) < heuristic_astar(map, end, existingNode)) {
    ...
    private static List<AStarNode> neighbors_astar(final GameMap map, final AStarNode node) {
        List<AStarNode> neighbors = new ArrayList<>();
        neighbors.add(new AStarNode(map.getLocation(node.loc, Direction.NORTH), node));
        neighbors.add(new AStarNode(map.getLocation(node.loc, Direction.EAST), node));
        neighbors.add(new AStarNode(map.getLocation(node.loc, Direction.SOUTH), node));
        neighbors.add(new AStarNode(map.getLocation(node.loc, Direction.WEST), node));
        return neighbors;
    }

    private static double heuristic_astar(final GameMap map, final Location end, final AStarNode current) {
        return current.getPathValue() + map.getDistance(current.loc, end);
    }

    private static class AStarNode {
        public Location loc;
        public AStarNode parent;
        private double pathValue; // The total value so far along this path

        public AStarNode(Location loc, AStarNode parent) {
            this.loc = loc;
            this.parent = parent;
        }

        public double getPathValue() {
            if (parent == null) return loc.getSite().strength;
            return parent.getPathValue() + loc.getSite().strength/255.0 + 1;
            // TODO: try lessening the strength contribution
        }

        public boolean equals(Object o) {
            if (o == null || !(o instanceof AStarNode)) return false;
